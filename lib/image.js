...
exports.generateNine = function(id, w, h, scale, x, y, W, tpl) {
    ...
    .then(()=>runCommand(null, `mkdir -p ${path.join(__dirname, '../thumb', id, buildId)}`))
    .then(function(){
        return Promise.all([1,2,3,4,5,6,7,8,9].map(function(i){
            return sharp(path.join(__dirname, '../data', id, buildId, i+'.png'))
            .resize(50,50)
            .toFile(path.join(__dirname, '../thumb', id, buildId, i+'.png'))
        }));
    })
    .then(function(){
        setTimeout(function(){
